...
from common.dataset.dataset_rips import RIPSDataset, RIPSMetaDataset
from common.dataset.build_tasks import Task, DL_Task


def rips_organize(args):
    '''
    返回任务级别的数据，
    ...
    synthetic_dframe = pd.read_csv(os.path.join(args.synthetic_data_csv))
    ...
    test_df_ori = pd.read_csv(os.path.join(args.project_path, args.val_csv, f't_{args.index_fold}_q.csv')) # rips 验证和测试一样
    ...
    for each_task in range(args.n_test_tasks):
        ...
        train_set = RIPSDataset(args, fileroots=task.support_roots)
        val_set = RIPSDataset(args, fileroots=task.query_roots, mode='val')
        test_set = RIPSDataset(args, fileroots=task.test_roots, mode='test') # test=val
    ...
    meta_train_support_loader = DataLoader(RIPSMetaDataset(args, meta_train_support_tasks), 
                                      batch_size=args.meta_size, num_workers=0, pin_memory=True, shuffle=True)
    
    meta_train_query_loader = DataLoader(RIPSMetaDataset(args, meta_train_query_tasks),
                                    batch_size=args.meta_size, shuffle=True, num_workers=0, pin_memory=True)
    

    meta_val_support_loader = DataLoader(RIPSMetaDataset(args, meta_val_support_tasks),
                                     batch_size=args.meta_size, shuffle=True, num_workers=0, pin_memory=True)
    
    meta_val_query_loader = DataLoader(RIPSMetaDataset(args, meta_val_query_tasks),
                                   batch_size=args.meta_size, shuffle=True, num_workers=0, pin_memory=True)
...
def rips_organize_tl(args):
    ...
    train_df_ori = pd.read_csv(os.path.join(args.project_path, args.train_csv, f't_{args.index_fold}_s.csv'))
    val_df_ori = pd.read_csv(os.path.join(args.project_path, args.val_csv, f't_{args.index_fold}_q.csv'))
    test_df_ori = val_df_ori # rips 没有验证集
    ...
    for each_task in range(args.n_test_tasks):
        ...
        train_set = RIPSDataset(args, fileroots=task.support_roots, is_sys=True)
        # val_set = BV1000_OCT_CNV_Sys(args, fileroots=task.query_roots, mode='val')
        val_set = RIPSDataset(args, fileroots=task.query_roots, mode='val') # 使用真实图片验证
        test_set = RIPSDataset(args, fileroots=task.test_roots, mode='test', is_sys=True)
