...
parser.add_argument('--dataset', type=str, default='cifar10', help='cifar10 | lsun | mnist')
...
parser.add_argument('--output', default='result', help='folder to output images and model checkpoints')
...
for epoch in range(opt.epoch): 
	for i, (imgs, labels) in enumerate(dataloader): 
		...
		reall = Variable(FT_a(batch_size,1).fill_(real_label))#([64,1])
		...
		noise = Variable(FT_a(np.random.normal(0, 1,(batch_size, opt.latentdim)))) # ([64,100])
		gen_labels = Variable(FT(np.random.randint(0, opt.n_classes, batch_size))) # ([64])
...
def generate_latent_points(latent_dim, n_samples, n_classes):
    ...
    ### START CODE HERE
    # 随机生成向量和标签，作为测试使用
    z = Variable(torch.FloatTensor(np.random.normal(0, 1, (n_samples, latent_dim))))
    gen_labels = Variable(torch.LongTensor(np.random.randint(0, n_classes, n_samples)))
    ### END CODE HERE
